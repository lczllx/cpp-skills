...
**异常安全(exception safety)的三个保证？** ⭐⭐⭐

1. 基本保证(basic guarantee)：操作失败时对象仍处于有效状态（无资源泄漏、不变量保持）
2. 强保证(strong guarantee)：操作失败时回滚到操作前状态（如 commit-or-rollback）
3. 无抛出保证(nothrow guarantee)：操作绝不抛异常（析构函数、移动构造、swap 应为 noexcept）

追问：为什么移动构造函数应标记 noexcept？（使 vector 扩容时可以安全移动而不是拷贝，否则退化为拷贝以保证强异常安全）

**拷贝消除(copy elision)与 RVO/NRVO？** ⭐⭐

C++17 起某些场景强制拷贝消除：返回纯右值时直接在返回值存储位置构造（不经过拷贝/移动）。RVO(返回值优化)：返回局部变量时消除拷贝。NRVO(命名返回值优化)：返回命名局部变量时消除拷贝。追问：为什么有 RVO 还要写移动构造？（RVO 不总是触发，分支返回时无效，且标准不强制 NRVO）
...
Pointer to IMPLementation — 将类的实现细节移到单独的实现类中，公开类只持有指向实现类的指针（通常用 unique_ptr）。优点：编译隔离（修改实现不触发重编译）、ABI 兼容、隐藏依赖。代价：额外间接访问、多一次内存分配。追问：PIMPL 中 unique_ptr 的析构问题？（需要在 .cpp 中定义析构函数，因为 unique_ptr 的默认删除器要求实现类此时必须是完整类型）

**CRTP 是什么？解决了什么问题？** ⭐⭐

Curiously Recurring Template Pattern — 派生类作为基类的模板参数：`template<typename T> class Base { ... }; class Derived : public Base<Derived> { ... };`。实现编译期多态（静态多态），消除虚函数调用开销。典型应用：`std::enable_shared_from_this`、Eigen 库的矩阵运算。追问：CRTP 和虚函数的取舍？（CRTP 零运行时开销但不能动态分发，不能放入同一容器；虚函数可统一处理但每次调用有间接开销）

**类型擦除(type erasure)的原理？** ⭐⭐

将不同类型的对象包装为统一接口（如 std::function、std::any）。典型实现：外部类持有指向内部模板类的基类指针（基于虚函数），在赋值时构造具体类型的派生类对象。追问：类型擦除的开销？（虚函数调用 + 可能堆分配，比 CRTP 慢但灵活）
...
追问：为什么有的哈希表实现在链表过长时转红黑树（如 Java HashMap 链表长度 > 8）？（防止哈希碰撞攻击导致退化到 O(N)。C++ std::unordered_map 未强制此优化，但某些实现可能类似处理）
...
函数重载、命名空间、模板等导致相同函数名需要区分 → 编译器生成包含签名信息的修饰名。nm 查看，c++filt 反解析。extern "C" 禁止 mangling。追问：C 和 C++ 混合编程的关键？（头文件中用 `#ifdef __cplusplus extern "C" { ... } #endif` 包裹 C 函数声明，确保符号表兼容）
...
2. 线程 TID 转十六进制 → `gdb -p <pid> -batch -ex "thread apply all bt"` 或 `perf top -t <tid>` 定位热点函数
3. 火焰图分析调用链：`perf record -g -p <pid> -- sleep 30` → `perf script` 生成火焰图
4. 可能原因：死循环、自旋锁忙等（lock-free 退化为活锁）、正则回溯、频繁 malloc/free 导致分配器锁竞争
...
5. 检查内存分配器锁竞争（glibc malloc 的 arena 锁），考虑使用 jemalloc/tcmalloc + heap profiling
